...
**1. Single source of truth?** `payment_line_items` is the real ledger; `invoices.balance_due` and `payment_schedules.total_amount` are both denormalized caches. Post-TR-M2/TR-M3, the two invoice/payment total computations deliberately share identical logic — good. **Correction, 2026-07-17 (Commitment Alignment Sprint):** the claim that `updateScheduleTotalAmount` has zero callers is stale — Phase 3c reintroduced it with two real callers (`regeneratePaymentSchedule`, `addReviewInstallment`) that resolve exactly the "Over-allocated" disagreement described below by making `total_amount` genuinely agree with its line items again; the coordinator UI's warning remains as the signal that prompts a coordinator to use them, not evidence of an unreconciled gap. Separately, `invoices.balance_due` is also confirmed to have two independent app-level writers (`reconcileInvoiceBalance`, `recomputeInvoiceTotals`) computing identical logic by convention rather than a shared function — now backstopped by a DB-level trigger (`_trg_invoices_sync_balance_due` / `_trg_payment_line_items_sync_balance_due`) so it can never drift from `total` minus actually-collected payments regardless of which write path fires.
