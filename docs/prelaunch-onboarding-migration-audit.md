...
| Events/bookings | `createEvent()`, `lib/events/service.ts` | No dedup function; instead a hard **conflict guard** (`checkEventSpaceConflict()`, tag `TR-B1`) rejects a new event outright if it double-books a space/date | **Not in the Migration Center today.** See §12 for this initiative's decision. |
