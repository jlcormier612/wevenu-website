...
- **Task.** One checklist item inside a Booking's Planning, sourced from a Planning Template and then specific to that Booking. **Added 2026-07-16:** for a task acting on Client-Owned data, the task is not just a checklist item — it is the commit point where the couple's private work becomes visible to the venue. See `docs/client-workspace-product-architecture.md` §11 (Private Until Committed) for the full principle this implies for how Tasks should be designed.
