...
| Converting a Lead into a paying client | **Convert to Client** | Creates the Client (+ Event) commercial relationship. |
| Turning a calendar booking placeholder into a CRM record | **Convert to Lead** | Calendar placeholders open `/leads/new` — they create a Lead, not a Booking entity. |
...
| The analytics/reporting area | **Reports** | Matches the nav label (`Reports` → `/reporting`). Legacy `/analytics` redirects here. |
