...
- **Separate working OpenAI integrations** for observation/narration purposes (`luv-ask`, roll-ups, lead drafts via `lib/ai/openai.ts`), plus four more for a genuinely different purpose (transcribing pasted content — out of scope for this reconciliation, same as the prior document). A former unused client-drafts module was removed; do not assume a live client-detail draft UI.
