...
- `updateScheduleTotalAmount` deleted from `lib/payments/repository.ts` — it had zero callers (confirmed by repo-wide grep before removal), and the correct fix was removing manual total-editing entirely, not wiring the dead function up. **Correction, 2026-07-17 (Commitment Alignment Sprint):** this function was reintroduced in Phase 3c with two real, legitimate callers (`regeneratePaymentSchedule`, `addReviewInstallment`) — see `docs/domain-model.md` and `docs/architecture-audit.md` for the corrected current-state description; this bullet is kept as an accurate record of the Phase 1 decision at the time it was made.
