...
**Regression found and fixed during beta-readiness testing** (`20260713300000_fix_engagement_event_guard_regression.sql`): the guard above assumed every caller of `record_engagement_event()`/`check_relationship_milestones()` was a venue owner/team member or an HQ admin — but couples (`/p/{token}`, `/sign/{token}`) and vendors (claim/invitation flows) call these too, with no venue-staff session at all, so `current_user_venue_id()` is always null for them. The guard was raising "forbidden" on every couple portal-open, contract signing, and vendor invitation acceptance. Simulating an anonymous caller against both functions during readiness testing reproduced this immediately. Fix: `record_engagement_event()` now only enforces the ownership check when `p_actor_type` is `'venue_user'`, `'team_member'`, or `'hq_admin'` (the actor types with a real, checkable session identity) — couple/vendor calls are implicitly trusted the same way they were pre-Sprint-108.5, since the caller already validated a token via a separate function earlier in the request. `check_relationship_milestones()` dropped its guard entirely — it only re-derives celebrations from state that's already true and inserts idempotently, so there was never anything for the guard to protect. Verified after the fix: couple/vendor events succeed again, and the original cross-venue abuse case (an authenticated user claiming `actor_type: 'venue_user'` for a venue that isn't theirs) is still correctly blocked.
