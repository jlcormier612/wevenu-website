...
- **Calendar** — correctly *not* present as its own venue-wide item type, by explicit design (`docs/calendar-platform-integration.md` §2/§4). `timeline_entry` remains on `CalendarItemType` for filter sanitization only and is never aggregated into the venue-wide month grid. The former per-booking Booking Schedule surface that once rendered timeline entries has been retired.
