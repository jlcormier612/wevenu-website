...
| Payments (venue ↔ couple) | 🟡 Mostly complete | Live: double-mark-paid guard (Sprint 2), refund/void, hard-delete guards. Real online collection (Stripe Hosted Checkout — direction confirmed) still design-only, blocked on credentials this environment doesn't have (TR-M1) |
...
|---|---|---|
| Real Stripe payment collection (Hosted Checkout) | Deferred, designed | Direction confirmed (Hosted Checkout, overriding the earlier Payment Element design). Not started — its own major initiative with a dedicated architecture review before implementation, per explicit instruction. Full plan in `docs/stripe-launch-plan.md` |
