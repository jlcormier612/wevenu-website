...
**Date:** 2026-07-22 (updated after Sprint 2, the QuickBooks Online Launch Integration, Sprint 3, the build-restoration pass, and the RC1 launch-hardening pass)
...
- **Lead Acquisition & Intake:** replaced three independently-drifted lead-creation paths with one canonical pipeline (`ingest_lead`), a real registrable source vocabulary, a full audit trail (`lead_intake_attempts`), a verified reactivation policy (a repeat inquiry always creates a fresh Lead on the same Relationship, never reopens a closed one), and a real generic external-source channel — the Email Intake Engine, where a venue forwards inquiry emails from *anywhere* (The Knot, WeddingWire, their own inbox) to a per-venue address for OpenAI-based extraction via `lib/ai/openai.ts`. Full detail: `docs/lead-intake-final-report.md`.
...
- **Sprint 3 — Tour Scheduling, Email Intake, Facebook Lead Ads, QR Lead Capture:** four approved lead-acquisition initiatives, each assessed/designed then built and live-verified independently (kept fully separate per explicit instruction, not bundled). Tour Scheduling replaced "hours inferred from general business hours" with a real weekly recurring schedule, multiple windows per day, and blocked-date exceptions, unifying every slot-blocking check (public widget, coordinator booking, write-time re-checks) into one canonical function — a real inconsistency between the display-time and write-time checks was found and fixed in the process. Email Intake shipped full self-service onboarding, but more importantly, re-verifying it after a proxy fix exposed and fixed two genuine launch-blocking bugs (new-venue creation was silently failing; the webhook route was unreachable) plus a third found via that exposure (a missing `service_role` grant). Facebook Lead Ads shipped a full OAuth/webhook/retry-queue integration mirroring QuickBooks's own patterns, and its build found a real idempotency gap in the *shared* Lead Intake pipeline itself (a redelivered webhook could still create a duplicate Lead) — fixed at the pipeline level, not just for Facebook. QR Lead Capture shipped scan→record→redirect→attribution→analytics end-to-end. Full detail per item in their respective assessment/design docs plus this sprint's own live-verification passes.
- **Build Restoration:** `next build` had regressed to failing outright — an untracked `shared/` support directory (real infrastructure for the separate `workspace`/`marketing` apps, not part of this app) was being swept into the main app's TypeScript compilation by an overly broad, un-excluded `tsconfig.json` glob. Root-caused (one missing string in an `exclude` array, not a structural problem), fixed, and verified four ways — `tsc`/`next build` both clean, `workspace`'s own build and dev server unaffected, `shared/relationships`'s own smoke test actually run (19/19 passed). Full detail: `docs/build-restoration-report.md`.
- **RC1 Launch-Hardening Pass:** four items completed before allowing Stripe to begin as its own initiative, per explicit instruction. Notification UX polish — removed internal-mechanics language ("Notification Engine," "process reminders," "pending") from the one remaining Settings surface still using it, reframed around delivery health a venue actually cares about (is email working, did it send, is anything failing), backend architecture untouched. QuickBooks "Retry now" manual re-sync — the one piece explicitly deferred from `docs/quickbooks-online-architecture.md`, now built and live-verified (a real dead-lettered item correctly reset to a fresh attempt budget and re-processed; the session-client write path confirmed authorized via a real signed JWT, not just the superuser client), completing all 12 steps of the QuickBooks Financial Validation checklist. QR tour-booking attribution — the disclosed follow-up from the original QR Capture build, extending `book_tour()`'s SQL signature (a real overload-vs-replace gotcha found and fixed live: a bare `create or replace function` with a new trailing parameter silently creates a second function instead of replacing the original). Documentation reconciled across `docs/quickbooks-online-architecture.md`, `docs/rc-launch-validation-runbook.md`, `docs/qr-lead-capture-design.md`, and this file.
...
- Luv's Daily Briefing, full six-kind Observation Model narration convergence across its OpenAI-backed integrations, extension into Floor Plans and aggregate venue-facing Guests/Seating, progressive-disclosure controls for the couple/vendor audience.
