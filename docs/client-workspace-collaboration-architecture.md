...
## 10. Private Until Committed — current-state conflict inventory

Added 2026-07-16 alongside `docs/client-workspace-product-architecture.md` §11, which states the principle this section audits against: the venue's view of Client-Owned or Shared data should update only at a deliberate commit point, never continuously while the couple is still working. This section is the ground-truth evidence for that principle's "Conflicts with current implementation" list — same evidentiary standard as the rest of this document (file/line citations, no recommendations).

| Surface | Current behavior | Commit gate today? |
|---|---|---|
| Timeline (`timeline_entries`) | Any row tagged `client_editable` writes directly to the live table on save; if also tagged `guest`, the same write is immediately readable by the public wedding website and, via the Venue Workspace's Timeline editor, by the coordinator. One table, one row, no draft copy. | **None.** No status field, no "publish"/"approve" step exists anywhere on `timeline_entries`. **Target model adopted 2026-07-16, implementation paused pending it:** see `docs/client-workspace-product-architecture.md` §12 — three independent per-item axes (Owner/Lock State/Visibility) plus a whole-timeline submit-to-venue action, replacing the current `audiences`/`client_editable` pair. |
| Vendor recommendations (`EventVendorRecommendation`) | `select_event_vendor_recommendation` RPC writes the couple's choice directly onto the shared record the moment it's called — per §3.3 above, "the selection is visible to the venue immediately." Each category (florist, DJ, caterer, ...) is its own independent recommendation record with its own independent selection state. | **None per-category, and no cross-category batch step.** There is no "submit vendor list" action anywhere that reveals multiple selections at once; each is live the instant it's made. |
| Feature-adoption / couple-engagement analytics (`lib/analytics/types.ts`, consumed by `lib/luv/roll-up-service.ts`) | `seatingStarted`, `guestsAdded`, `rsvpCompletionAvg`, `rsvpRate`, `budgetConfigured` are computed venue-wide (aggregated across all of a venue's couples) and narrated into the venue owner's weekly Luv roll-up digest, e.g. "Seating: 3 started (60%)." No couple is named individually in this surface. | **Aggregate-only visibility, not per-couple, and not gated at all** — the underlying signal (has this couple touched their private canvas) is counted continuously as couples work, regardless of whether any of them has reached a stopping point. |
| Seating (`guest_seat_assignments`) | Already correctly gated *today* — the couple's own portal canvas is the only write path, and (per `docs/seating-release-readiness-final-assessment.md`) no coordinator-facing tool can read or write individual seat assignments at all; `wedding-day-seating.tsx` is explicitly read-only and, per that assessment, exists for day-of use, not for watching the couple plan. | **Not a current conflict** — flagged here only because the *paused* Product Completion item ("coordinator seat assignment," same assessment, Finding 1) would have introduced exactly this kind of live view if built as originally scoped, without an explicit commit/handoff gate. |
| Guest list (`couple_guests`) | No per-guest fields are read anywhere in the Venue Workspace — confirmed by grep, matches the existing "reference implementation" claim in §4 above. `events.guest_count` (the number a coordinator actually sees) is a separately-editable column, not a live aggregate of `couple_guests` rows. | **Not a conflict at the record level** — though this is a side effect of the guest-count triplication finding (`docs/booking-financial-architecture-final-release-assessment.md` Finding 1), not a deliberate Private-Until-Committed design. |
| Documents (`couple_documents.share_with_venue`) | A boolean set at upload/edit time by the couple; the Venue Workspace's document query only ever includes rows where this flag is true. | **Effectively gated**, because a document upload has no meaningful "in-progress" state to observe — the flag functions as a commit point by the nature of the data, not by explicit design against this principle. |
| Payments | Portal is view-only; no online payment path exists (§8, "Payments" row: "Client Only (view)"). | **Not applicable yet** — nothing exists today for the venue to prematurely observe. |
