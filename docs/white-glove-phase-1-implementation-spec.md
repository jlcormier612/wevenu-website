...
- **C:** every extraction failure (missing `OPENAI_API_KEY`, unreadable scanned-image PDF, etc.) falls back to the same existing, already-implemented non-AI behavior each pipeline already has (deterministic line-split for checklists, `plainTemplateSplit` for messages, existing Import Wizard error states for packages) — no new failure mode introduced.
