...
- **Narration is not hypothetical — it's already built for several separate purposes.** `luv-ask` (couple Q&A, Venue Guide-scoped), the dashboard roll-up (`lib/luv/roll-up-service.ts`, venue analytics → 4-quadrant narrative), and lead draft generation (`lib/luv/drafts.ts`) are genuinely separate, working Venue OpenAI integrations (via `lib/ai/openai.ts`), each with its own prompt-building logic. None of them currently narrates Event Readiness or the trust-tiered observation model this document defines. The natural next seam is not "build narration" — it's **converging these onto one shared narration mechanism** that takes trust-tiered input (§6) and produces a warm sentence, so a further purpose (narrating Event Readiness) doesn't become another independent implementation.
