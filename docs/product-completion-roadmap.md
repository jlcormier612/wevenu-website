...
**Update, 2026-07-16 — renamed the Commitment Alignment Sprint, resumed under `docs/commitment-lifecycle-architecture.md` (Approved).** Implementation order: Guest List Submission → Seating Delegation & Submission → Vendor Selection Submission → Booking Financial alignment items → Invoice/Payment/Document alignment items. Timeline remains intentionally paused pending its own already-designed target model. See that document's §9 Domain Mapping Matrix for the governing scope of each item.

**Update, 2026-07-17 — Commitment Alignment Sprint complete.** All five items shipped and live-validated; see `docs/commitment-alignment-sprint-final-report.md` for the full closing summary and `docs/commitment-alignment-sprint-retrospective.md` for the retrospective. Timeline remains the one deliberately-paused piece of this architecture, unchanged by the sprint's completion. Next work resumes from the Product Completion Roadmap's Programs below, informed by the sprint's findings.

**Update, 2026-07-17 — Timeline Implementation complete.** The one domain the Commitment Lifecycle's own Domain Mapping Matrix (§9) named as deliberately unbuilt is now shipped and live-validated (26/26 checks) — Owner/Lock State/Visibility/Submission model, Copy at Commitment, Visibility-follows-Ownership. See `docs/timeline-implementation-report.md` for the full build record. **The platform architecture is now considered stable.** Per explicit direction: unless future implementation uncovers a genuine contradiction, no further platform-wide architectural principles are to be introduced — remaining work builds and refines against the approved architecture (Workspace Sovereignty, Commitment Lifecycle, Publication, Copy at Commitment, One Canonical Owner, Visibility-follows-Ownership) rather than extending it. **This closes the architecture-definition arc that began with the original Product Completion Sprint pause.** Current status, re-scored against everything shipped since this document's 2026-07-07 baseline, now lives in `docs/release-readiness-status.md` — that document, not the Gap Analysis/Scorecard below, is the current working source of truth going into the refinement phase. The sections below are kept as-is for history and are not being re-verified line by line.

*(Original pause note, kept for history:)* Product Completion Sprint paused, then governed by `docs/commitment-lifecycle-architecture.md`. A more specific completion backlog was assembled from three later release-readiness assessments (`docs/key-dates-release-readiness-assessment.md`, `docs/seating-release-readiness-final-assessment.md`, `docs/booking-financial-architecture-final-release-assessment.md`) and begun as a sprint under this roadmap's Program 2/Program 1 follow-up work. It was paused before any implementation shipped, across three same-day corrections that turned out to be one pattern: **Private Until Committed** (Client Portal is private by default) → a **Timeline architecture correction** (one authoritative source, audience-specific published views) → **the Commitment Lifecycle Architecture** (`docs/commitment-lifecycle-architecture.md`), the platform-wide generalization of both, now a foundational document alongside Product Promise, Domain Model, Client Workspace, Hosted Experience Platform, Event Order, and Booking Financial Architecture. That document's §9 Domain Mapping Matrix and §11 Recommended Roadmap Adjustments supersede this note as the authoritative scope for resuming the sprint — concretely: Seating's "coordinator seat assignment" becomes a Delegation build, the guest-count-triplication finding and Key Dates' couple-visibility question both fold into the Guest List row of that matrix, and Timeline implementation stays paused pending its own already-designed target model. Two pieces already shipped in the Hosted Experience Platform initiative (Phase 3's schedule-sync behavior, Phase 5's change-notification nudge) were flagged for re-verification, not reverted, in that spec's status line. The sprint resumes once `docs/commitment-lifecycle-architecture.md` is reconciled against, not abandoned.
...
| 3 — Customer Experience | What couples/vendors feel | Couple Portal, White labeling (**Venue Brand Experience** — see below), Onboarding, Mobile polish, Notifications |
...
**`docs/domain-model.md`** is the conceptual model these four principles now live inside — the full set of business objects Wevenu is built from (Venue, Lead, Client, Event, Conversation, Asset, Contract, Invoice, Payment, Calendar Entry, Task, Vendor, Team Member, Contact, Luv Observation), each with what owns it, its lifecycle, and its canonical source of truth today. Program 2 implementation should be checked against that document before new schema is written, the same way a feature is checked against the Product Promise before it ships.

**`docs/program-2-implementation-plan.md`** is the implementation plan for Program 2's five priorities (Conversation, Assets, Calendar, Lead Lifecycle, and the "cohesion before features" review discipline) — target architecture, migration strategy, order of implementation, risks, simplification opportunities, and Trust Risks closed for each. Proposal stage — reviewed and agreed before any Program 2 code is written, same as every other design document this project has produced.

**North Star (adopted 2026-07-21, sits above every principle below): every completed Program 2 phase should reduce cognitive load for the venue owner.** The architecture is allowed to become more sophisticated with every phase; the product experience must become simpler with every phase. These customers are barn owners, farm owners, winery owners, family businesses, independent venues — not technical users. They think "I got a lead," "I'm trying to book them," "I'm planning their event," "I'm running event day" — never "Relationship," "Opportunity," "Conversation," "Asset." That's the mental model every UI decision has to preserve, regardless of how correct the underlying model is. When a principle below and this one seem to pull in different directions, this one wins.

**Program 2 — six adopted design principles (2026-07-07, #5 added 2026-07-21, #6 added 2026-07-21 alongside the North Star above):**
...
**5. The venue should experience customers, not architecture — Relationship, Opportunity, Conversation, and Asset are the correct internal model, but a coordinator should just feel like they're opening "Emma & James" and everything they need is there.** Surfaced explicitly once Conversation implementation was underway (Program 2 Phase 2B): it's possible to get the architecture exactly right — Relationship as the enduring identity, Lead as an Opportunity within it, Conversation anchored correctly — and still build a UI that makes a coordinator think in those terms, which would be a failure dressed as a success. The target isn't a set of screens being replaced one at a time (a Messages tab here, a Conversation view there); it's a single **Relationship Workspace** — the one place a venue naturally starts every interaction with a customer, where the underlying model has almost entirely disappeared behind "here's everything about Emma & James." Every remaining Program 2 phase (Assets, Activity Timeline, the eventual Conversation/Assets integration) should be evaluated against whether it strengthens that one workspace, not just whether it's architecturally correct in isolation.

**6. Progressive disclosure — show venues only what they need right now, and reveal complexity only when they ask for it.** The platform underneath is allowed to keep getting more sophisticated (Relationship, Opportunity, Conversation, Asset, Activity Timeline all coexisting correctly); the day-to-day experience should get calmer and more obvious with every phase, not busier. A venue owner shouldn't see a field, a tab, or a concept until the moment they actually need it — the correct architecture existing is not, by itself, a reason to surface it.

All six principles deserve the same "design before code" pass TR-M1 got (`docs/stripe-payment-architecture.md`) once Program 1 closes and Program 2 starts in earnest. Principles 1-4 captured here as adopted direction ahead of design; principles 5-6 and the North Star emerged mid-implementation and are already shaping Phase 2B (see `docs/conversation-experience-cutover.md` and `docs/program-2-implementation-plan.md`).
...
| 8 | Calendar | 🟢 Green *(TR-B1 Resolved 2026-07-07; TR-B4 + TR-B5 Resolved 2026-07-08 — Program 2 Phase 1)* | Double-booking is server-enforced. Tours now reliably appear regardless of entry point — `tour_appointments` is the single canonical source, the legacy `leads.tour_date` fields were backfilled and dropped, and expired holds no longer linger past their expiry. No team visibility on the grid and month-only view remain Operational Completeness, not Trust Risks |
...
**Approved future initiative, 2026-07-17 — Venue Brand Experience.** A full architectural audit (`docs/venue-branding-architecture-audit.md`) confirmed and extended the findings above, and the venue's existing onboarding-time Venue Style (`venue_type`) was approved as the single canonical input into a new brand recommendation engine — generating suggested Collections, Color Stories, Typography, Gallery Style, and Motion (Hosted Experience Platform), plus Client Portal, email, PDF, and guest-facing defaults — reviewable/overridable, never a second self-classification step. See that document's "Future Initiative: Venue Brand Experience" section for the full scope. Not scheduled; not part of the Commitment Alignment Sprint currently in progress.

### 8. Calendar — 🟢 Green *(TR-B1 Resolved 2026-07-07; TR-B4 + TR-B5 Resolved 2026-07-08, Program 2 Phase 1 — see register)*
...
- ~~The "tour" source in that 7-way aggregation read only the legacy `leads.tour_date` field~~ — **fixed:** `tour_appointments` is now the canonical source for tours regardless of entry point; the legacy fields were backfilled and dropped, not left as a second representation. The tour source itself moved into a dedicated projection function (`lib/tours/service.ts`'s `getTourCalendarEntries`), the first real instance of the "Calendar composes, domains own their projection" pattern the Program 2 principle called for. See `docs/trust-risk-register.md` TR-B4/TR-B5.
...
- The `proposal_sent` pipeline stage is a status value with no artifact behind it — no pricing, packages, revisions, or client acceptance. **Approved future initiative, 2026-07-17 — Commercial Proposal Architecture** (`docs/future-initiative-commercial-proposal-architecture.md`): a formal Proposal artifact bridging Sales CRM and Booking, deliberately kept outside the Commitment Lifecycle (it's a pre-commitment commercial artifact, not a Commitment Lifecycle one). Not scheduled; not part of the Commitment Alignment Sprint.
