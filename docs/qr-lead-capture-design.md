...
1. ~~Threading `?qr=` through existing destination pages (§3)~~ — **Resolved.** `app/form/[key]` (via `components/form/inquiry-form.tsx`) shipped with the original QR Capture build. `app/book/[key]` shipped 2026-07-22 as a follow-up: `book_tour()`'s SQL signature gained an optional `p_qr_campaign_id` parameter (a real gotcha found live — a plain `create or replace function` with a new trailing parameter silently creates a second overloaded function instead of replacing the original; the old signature had to be explicitly dropped first), threaded through `lib/tours/service.ts` → `app/api/tours/book/route.ts` → `components/tours/tour-scheduler.tsx` (reads `?qr=` from the URL at submit time). Live-verified: a real QR-attributed tour booking produced a Lead with `source_data->>'qr_campaign_id'` set correctly and counted in that campaign's analytics; a booking with no `?qr=` param still works unchanged (backward compatible).
