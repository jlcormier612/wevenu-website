...
| Luv drafting — **already real for the Lead stage**: `follow_up_email`, `follow_up_text` (SMS drafting is already anticipated in the type, even though SMS can't be sent yet), `next_steps`, `timeline` for Leads. Coordinator reviews, edits, sends manually — Luv never sends. | `lib/luv/drafts.ts`, `components/luv/luv-draft-panel.tsx` | This *is* "Ask Luv to Draft" and half of "Luv suggests templates" already — extend it, don't reinvent it |
