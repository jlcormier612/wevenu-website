...
**Lifecycle:** `pending` → (optionally `blocked` on a dependency) → `pending`/`overdue` → `complete` (or `waived`). Completion can be manual (a person checks it off) or automatic, via a named trigger (`contract_signed`, `questionnaire_submitted`, `timeline_created`, and others — see the caution below). **Per `docs/commitment-lifecycle-architecture.md` §2/§8 (2026-07-16):** for a Task acting on Client-Owned data, this auto-complete pattern *is* the platform-level Submit event — the task's completion should be the commit point, not a separate signal about one. `contract_signed`/`questionnaire_submitted` are the working precedent this generalizes from.
...
**Canonical source of truth:** `payment_schedules` + `payment_line_items`, with `refunded_amount` tracked as a running total against the original `paid_amount` (TR-M3). **Correction, 2026-07-17 (Commitment Alignment Sprint):** the claim below that `updateScheduleTotalAmount` has zero callers is stale — Phase 3c reintroduced it with two real, legitimate callers (`regeneratePaymentSchedule`, `addReviewInstallment`), both resolving a Needs Review state by making `total_amount` genuinely agree with its line items again. **Remaining known gap:** the couple portal's payment view computes its own totals with a filter that predates refund support, so a couple's displayed "amount paid" can already be wrong the moment a refund happens — not fixed yet. Separately, `invoices.balance_due` now has a DB-level trigger (`_trg_invoices_sync_balance_due` / `_trg_payment_line_items_sync_balance_due`) enforcing it can never drift from `total` minus actually-collected payments, regardless of which app-level code path wrote to either side.
