...
**Represents:** The full history of communication between the venue and one Client (or, eventually, one Vendor) — everything said, across every channel, in one continuous thread. **Adopted principle (Program 2):** email, SMS, portal chat, vendor chat, internal notes, phone-call logs, voicemail, and push are transports; the actual object is the Conversation, with Messages flowing through pluggable Channels.

**Owned by:** Jointly the venue and the Client — both sides read and write to it (a Conversation is the one entity in this model that's inherently bilateral by nature, not primarily owned by one side with the other granted access).

**Lifecycle:** Begins on first contact → accumulates Messages indefinitely → never closes (a Conversation with a past client is still a Conversation, just quiet).

**Relationships:** Belongs to exactly one Client (or, in the target model, one Vendor relationship). Contains many Messages, each attributed to a sender (venue Team Member or Client/Contact) and a Channel.

**Canonical source of truth:** *Not yet unified — this is the clearest gap between this model and today's implementation.* Two entirely disconnected systems currently each hold half of what should be one Conversation: `message_threads`/`messages` (an outbound-email log, keyed to `client_id`) and `couple_threads`/`couple_messages` (real bidirectional portal chat, also keyed to `client_id`, but never joined to the first system anywhere in the code). This is registered as TR-C1 — Identified, explicitly not a same-day patch, and is the single largest architectural project this domain model implies.
