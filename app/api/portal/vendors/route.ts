...
// (/api/portal/vendors/submit). Directory picks use
// toggle_directory_vendor_pick (same private picked_at, upserts a
// source='couple' recommendation when needed).
...
export async function POST(request: Request) {
  ...
  const { token, clientId, recommendationId, vendorId, picked } = body;
  if (!token || !clientId || picked === undefined) {
