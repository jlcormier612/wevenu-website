 ...
 *
 * Couple questionnaire submission. Supports familyPayload jsonb and legacy columns.
 */
...
export async function POST(request: NextRequest) {
  ...
  if (body.familyPayload && typeof body.familyPayload === "object") {
    const { data, error } = await supabase.rpc("submit_questionnaire_family_as_couple", {
      p_key: accessKey,
      p_payload: body.familyPayload,
      p_expected_updated_at: expectedUpdatedAt,
    });
    if (error || !data?.ok) {
      return NextResponse.json({ ok: false, error: data?.error, message: data?.message ?? data?.error ?? "Could not submit." }, { status: 400 });
    }
