/**
 * Calendar Slice 2A.2.3 — live DB checks for custom label uniqueness and cap.
 */
...
const CASES = resolve("lib/calendar/venue-schedule-catalog-2a23.db.sql");
...
describe("venue_schedule_item_types custom CRUD live database", () => {
  it("enforces label uniqueness, rename isolation, archive reuse, and active cap", (t: TestContext) => {
    if (!localDbAvailable()) {
    ...
    if (hasCatalog.status !== 0 || !/t/.test(hasCatalog.stdout)) {
      // Fresh DB: apply foundation migration once, then the 2A.2.3 index.
      applySql(CATALOG);
    }
