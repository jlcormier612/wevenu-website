...
type BlockRow = { id: string; venue_id: string; title: string; reason: CalendarBlock["reason"]; start_date: string; end_date: string; is_all_day: boolean; start_time: string | null; end_time: string | null; notes: string | null; recurrence_rule: string; recurrence_ends_on: string | null; created_at: string; };
...
export async function insertBlock(client: DbClient, venueId: string, input: CalendarBlockInput): Promise<string> {
  const { data, error } = await client.from("calendar_blocks")
    .insert({ venue_id: venueId, title: input.title.trim(), reason: input.reason, start_date: input.startDate, end_date: input.endDate || input.startDate, is_all_day: input.isAllDay, start_time: (!input.isAllDay && input.startTime) ? input.startTime : null, end_time: (!input.isAllDay && input.endTime) ? input.endTime : null, notes: input.notes.trim() || null, recurrence_rule: input.recurrenceRule ?? "none", recurrence_ends_on: (input.recurrenceRule && input.recurrenceRule !== "none" && input.recurrenceEndsOn) ? input.recurrenceEndsOn : null })
    .select("id").single<{ id: string }>();
  if (error) throw error;
  return data.id;
}
