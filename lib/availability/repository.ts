...
type BlockRow = { id: string; venue_id: string; title: string; reason: CalendarBlock["reason"]; start_date: string; end_date: string; is_all_day: boolean; start_time: string | null; end_time: string | null; notes: string | null; created_at: string; };
...
const mapBlock = (r: BlockRow): CalendarBlock => ({ id: r.id, venueId: r.venue_id, title: r.title, reason: r.reason, startDate: r.start_date, endDate: r.end_date, isAllDay: r.is_all_day, startTime: r.start_time?.slice(0, 5) ?? null, endTime: r.end_time?.slice(0, 5) ?? null, notes: r.notes, createdAt: r.created_at });
...
export async function insertBlock(client: DbClient, venueId: string, input: CalendarBlockInput): Promise<string> {
  const { data, error } = await client.from("calendar_blocks")
    .insert({ venue_id: venueId, title: input.title.trim(), reason: input.reason, start_date: input.startDate, end_date: input.endDate || input.startDate, is_all_day: input.isAllDay, start_time: (!input.isAllDay && input.startTime) ? input.startTime : null, end_time: (!input.isAllDay && input.endTime) ? input.endTime : null, notes: input.notes.trim() || null })
    .select("id").single<{ id: string }>();
  if (error) throw error;
  return data.id;
}
...
export async function checkAvailability(
  ...
  if (blocks && blocks.length > 0) {
    conflicts.push({ type: "calendar_blocked", message: `Date is blocked: ${(blocks[0] as { title: string }).title}`, severity: "warning" });
  }
