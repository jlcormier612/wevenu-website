...
export const SETUP_GUIDES: readonly SetupGuide[] = [
  ...
  guide({
    ...
    title: "Understanding your Calendar — Events, Tours, Holds, and Blocks",
    shortTitle: "Understanding Your Calendar",
    intro: "Your Calendar shows several different kinds of things, and they don't all work the same way. This guide answers the questions people actually ask — what stops a date from being double-booked, what each concept means, and what you need to configure before any of it works correctly.",
    ...
    steps: [
      {
        ...
        doThis: "A date is protected when you place a Hold and your availability settings say Holds prevent booking, or when you move a relationship to Booked. Blocked time and appointments protect a time only when that item is configured to block availability. A preferred date does not reserve a date.",
        lookFor: "If a date won't accept a new booking, check Booked events, Holds (and your Hold setting), and anything on the calendar configured to block availability.",
        dontDo: "Do not assume an inquiry or a preferred date has reserved the date.",
      },
      {
        number: 2,
        title: "What is a Tour / Appointment?",
        doThis: "A Tour is a scheduled visit — a lead coming to see your venue. It appears on your Calendar and counts against your tour scheduling capacity (how many tours you can host at once), which is separate from your event capacity. Booking a tour does not reserve the date for an eventual wedding — it only reserves that tour time slot. Choose whether tours can be scheduled when a booked event is occupying the date. This setting applies wherever Hello to Cheers checks tour availability.",
      ...
      },
      {
        ...
        doThis: "A Hold is a date you deliberately protect. Whether that Hold prevents another booking is your availability setting, and that setting applies wherever Hello to Cheers checks availability. A Hold is not a booking, and preparing a client workspace is not a Hold.",
        lookFor: "Settings → Availability shows whether Holds prevent booking.",
        dontDo: "Do not treat a preferred date as a Hold.",
      },
      {
        number: 4,
      ...
      },
      {
        ...
        doThis: "Migration Center can bring over Events, Tours, Holds, and Calendar Blocks (including recurring blocks) from your old system, with review before anything is created.",
    ...
    completion: "You know which calendar concepts actually protect a date (Events, Calendar Blocks), which ones don't (Tours, Holds), and how the two different \"hours\" settings fit in.",
    troubleshooting: [
  ...
  guide({
    ...
    steps: [
      ...
      { number: 7, title: "Know how messages actually go out", doThis: "Read the Communication setup guide next. Email and texting are set up for Hello to Cheers as a platform — you do not add email-provider or texting credentials in venue Settings. Once you are working in the workspace, open Inbox → Communication Health before sending a real message so you can see whether Email and texting are ready.", lookFor: "You know the four send choices — Email, Text, Portal message, and Internal note — and you know Communication Health is where readiness is shown." },
    ],
  ...
  guide({
    ...
    intro: "Hello to Cheers is the place you send messages to couples and vendors. The four send choices are Email, Text, Portal message, and Internal note. Email is set up for Hello to Cheers as a platform. Texting is enabled per venue in Settings → Communications. The compose box will tell you whether each channel is ready before you send.",
    ...
    steps: [
      { number: 1, title: "Open Inbox and Communication Health", doThis: "Go to Inbox. Use Communication Health (linked from the top of Inbox) to see whether Email and texting are ready. Email is platform-managed. Texting is turned on for your venue in Settings → Communications → Text messaging — you do not enter provider credentials. Your venue contact email and phone in Settings are for tests and for how couples reach you.", lookFor: "Communication Health shows Email configured and Texting configured as ready, or it explains that texting is not ready yet and links you to Enable text messaging.", dontDo: "Do not look for provider credential screens. Texting setup asks for your business details in plain language — not Account SIDs or API keys." },
      ...
      { number: 4, title: "Send a text", doThis: "Choose Text. Confirm the destination phone number. Write the message. Review the text preview. Choose Send text now only when texting permission is in place. If permission is missing or they opted out, the compose box explains why and Send text now stays unavailable.", lookFor: "The compose box shows the formatted phone number. If they have not given permission, the compose box explains that texting permission is collected on the inquiry and tour forms. If they opted out, it says texting is unavailable.", dontDo: "Do not try to send a text when Communication Health says texting is not set up, and do not send when they have not given permission or have opted out." },
      ...
      { number: 7, title: "Use a template without sending unresolved details", doThis: "For Email or Text, optionally pick a template. Hello to Cheers fills in the couple's details using the same rules as send time. Review the preview. If details are still missing, edit the message before you send.", lookFor: "The preview shows the actual wording that will be sent, not leftover {{tokens}} you have to guess about." },
      { number: 8, title: "Schedule Email or Text when you need to send later", doThis: "For Email or Text, choose Schedule, pick the date and time, and confirm. Scheduling is not available for Portal message or Internal note.", lookFor: "The conversation shows the message as scheduled for that time — not as already sent." },
      { number: 9, title: "Read delivery status and replies in the same conversation", doThis: "After an Email or Text is sent, the conversation shows whether it was sent and later delivered or failed. If a couple replies to an email you sent from this conversation, that reply lands in this same conversation. Texts they send back also land here.", lookFor: "You never have to hunt in a separate email inbox or a leftover messages list for a reply to a conversation email." },
    ],
    ...
    troubleshooting: [
      ...
      "If Text shows as not ready, open Settings → Communications → Text messaging (or the Enable text messaging link from Inbox / Communication Health) and complete setup for your venue. Portal message and Internal note still work.",
      ...
      "If you expected a couple to see a note and they cannot, check that you sent a Portal message, Email, or Text — not an Internal note.",
    ],
    relatedFeatures: [
      { label: "Open Inbox", href: "/messaging" },
      { label: "Open Communication Health", href: "/messaging/health" },
      { label: "Enable text messaging", href: "/settings/communications#texting" },
    ],
