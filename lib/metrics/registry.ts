...
export const METRIC_REGISTRY: MetricDefinition[] = [
  ...
  },
  {
    ...
    businessDefinition: "Lifecycle Bookings with a lead_id, grouped by leads.source. Direct/import without a lead are Unknown/Unattributed — never fabricated.",
    owner: "Venue", formula: "GROUP BY leads.source via lifecycle_booking_events.lead_id",
    sourceTables: ["lifecycle_booking_events", "leads"], dimensions: ["Lead Source"], filters: ["date range"],
    aggregationRules: "COUNT ... GROUP BY source; null/missing → unknown",
    unit: "count", precision: "integer",
    consumers: ["lib/metrics/lifecycle-booking.ts", "Sales cohort"], dependencies: ["Lifecycle Booking"], status: "canonical",
  },
  {
  ...
  // ── Conversion ──────────────────────────────────────────────────────────
  {
    ...
    businessDefinition: "Of leads that entered during a cohort window, the share that eventually received a Lifecycle first_booked (leads.first_booked_at). Not Financially Committed. Not period activity.",
    owner: "Venue", formula: "ROUND(100.0 * COUNT(leads with first_booked_at) / COUNT(leads created in window))",
    sourceTables: ["leads"], dimensions: ["Venue (implicit)", "Lead Source"], filters: ["lead created_at window"],
    aggregationRules: "Cohort only — never mix with financial booked_at.",
    unit: "%", precision: "integer (rounded)",
    consumers: ["lib/metrics/lifecycle-booking.ts:getLeadCohortLifecycleBookingStats"],
    dependencies: ["Lifecycle Booking"], status: "canonical",
  },
  ...([
