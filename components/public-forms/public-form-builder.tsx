    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {status === "published" && (
          ...
          >
            Preview public form <ExternalLink className="h-3 w-3" />
          </a>
        )}
      </div>
      ...
      {status === "published" && (
        <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
          <p className="font-medium text-heading">Public URL</p>
          <code className="mt-1 block break-all">{publicUrl}</code>
          <p className="mt-2">
            SMS consent is not collected on custom Public Forms. Phone is contact info only.
          </p>
        </div>
      )}
