...
export function EventNotesSection({ eventId, initialNotes }: { eventId: string; initialNotes: EventNote[] }) {
  ...
  function handleAdd() {
    ...
    startAdd(async () => {
      ...
      if (result.ok) {
        setNotes((p) => [{ id: crypto.randomUUID(), venueId: "", eventId, body: body.trim(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, ...p]);
  ...
  function handleSaveEdit(noteId: string) {
    ...
    startSave(async () => {
      const result = await updateEventNoteAction(noteId, eventId, editBody);
      if (result.ok) {
  ...
  async function handleDelete(noteId: string) {
    ...
    const result = await deleteEventNoteAction(noteId);
    setDeletingId(null);
            ...
            <div key={note.id} className="group relative rounded-lg border border-border bg-card p-4">
              ...
              <div className="mt-2 flex items-center justify-between gap-2">
                <p className="text-xs text-muted-foreground" title={new Date(note.createdAt).toLocaleString()}>
                  {relativeTime(note.createdAt)}{note.updatedAt !== note.createdAt ? " · edited" : ""}
                </p>
