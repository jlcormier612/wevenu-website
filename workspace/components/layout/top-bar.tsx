...
export function TopBar({
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[color-mix(in_srgb,var(--taupe-medium)_40%,transparent)] bg-[color-mix(in_srgb,var(--warm-gray)_92%,transparent)] backdrop-blur-sm">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-8 py-4">
        <p className="hidden text-sm text-[color-mix(in_srgb,var(--forest-sage)_65%,transparent)] md:block">
          One relationship. One timeline. One source of truth.
        </p>
        <div className="flex flex-1 items-center justify-end gap-4">
          <div className="hidden text-right lg:block">
            {unread.length === 0 ? (
              <p className="text-sm text-[var(--heritage-sage)]">No new alerts</p>
            ) : (
                ...
                {unread.map((n) => (
                  <li key={n.id} className="text-sm">
                    <Link
                      ...
                      className="text-[var(--forest-sage)] hover:text-[var(--heritage-sage)]"
                    >
                      {n.title}
                      <span className="ml-2 text-[color-mix(in_srgb,var(--forest-sage)_55%,transparent)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden min-w-0 sm:block">
              <p className="truncate text-sm font-medium text-[var(--forest-sage)]">
                {actor.name}
              </p>
            ...
            <form action={logoutAction}>
              <button
                type="submit"
                className="text-xs text-[var(--heritage-sage)] underline-offset-4 hover:underline"
              >
                Sign out
              </button>
            </form>
