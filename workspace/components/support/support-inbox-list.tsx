...
export function SupportInboxList({ items }: { items: SupportInboxItem[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {items.map((item) => (
        <article
          key={item.id}
          className="rounded-sm border border-[color-mix(in_srgb,var(--taupe-medium)_40%,transparent)] bg-[var(--true-white)] px-4 py-4"
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-sm text-[color-mix(in_oklch,var(--forest-sage)_80%,transparent)]">
                {[item.actorName, item.actorEmail].filter(Boolean).join(" · ") || "Unknown actor"}
              </p>
            </div>
            {item.status === "open" ? (
              <button
              ...
              >
                {isPending && pendingId === item.id ? "Resolving…" : "Resolve"}
              </button>
            ) : null}
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-[color-mix(in_oklch,var(--forest-sage)_65%,transparent)]">
            {item.rating != null ? <span>Rating {item.rating}/10</span> : null}
