...
# attempt (its SQL, not just its tracking row) — this records that fact and
# the loop below explicitly skips re-running that one file by name.
# ---------------------------------------------------------------------------
...
# The 5 known timestamp-collision groups — used only to label an expected bookkeeping
...
KNOWN_COLLISIONS="20261175000000 20261176000000 20261177000000 20261222000000 20261298000000"

# Already applied in the prior run — skip by exact filename, not by version, so the
# other files sharing a timestamp with any future collision are never accidentally skipped.
SKIP_FILE="supabase/migrations/20260626090000_venue_foundation.sql"

for f in $(ls supabase/migrations/*.sql | sort); do
  if [ "$f" = "$SKIP_FILE" ]; then
    echo "Skipping $(basename "$f") — already applied and recorded above."
    continue
  fi
  ...
  if [ -z "$insert_result" ]; then
    if echo "$KNOWN_COLLISIONS" | grep -qw "$version"; then
      echo "  (expected: version $version already tracked — one of the 4 known collision groups. $(basename "$f")'s SQL was fully applied above; schema_migrations can only hold one row per version.)"
    else
      echo "  !! UNEXPECTED: version $version was already tracked, and this timestamp is not one of the 4 known collision groups. $(basename "$f")'s SQL was still applied above, but this bookkeeping collision needs investigating — it may mean the script is re-running from a partial prior attempt, or a new, previously-unseen collision exists."
    fi
  fi
done
unset PGPASSWORD
