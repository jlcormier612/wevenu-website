import Image from "next/image";
import { JourneyNav } from "@/components/marketing/journey/journey-nav";
import { MarketingCta, WalkthroughCtas } from "@/components/marketing/marketing-cta";
import { FILM } from "@/lib/marketing/film";
import { EDITORIAL_BLEED, EDITORIAL_BREAK_Y, EDITORIAL_FRAME, EDITORIAL_IMAGE, TYPE_HERO_SHELL } from "@/lib/marketing/rhythm";

const PRINCIPLES = [
  {
    n: "01",
    title: "One Shared Workspace",
    body: "Everyone contributes to the same planning experience instead of maintaining separate versions.",
  },
  {
    n: "02",
    title: "Nothing Gets Lost",
    body: "Questions, decisions, notes, and updates stay attached to the booking instead of disappearing into email.",
  },
  {
    n: "03",
    title: "Progress Feels Natural",
    body: "Planning simply moves forward because everyone is working from the same information.",
  },
] as const;

const FLOW = [
  "Conversation",
  "Task Created",
  "Timeline Updated",
  "Couple Sees Change",
  "Venue Sees Progress",
  "Everyone Stays Aligned",
] as const;

type PlanningExperienceProps = {
  prev: { id: string; title: string };
  next: { id: string; title: string };
};

/**
 * Planning journey chapter — collaboration as hospitality.
 * Everyone planning together—not everyone planning separately.
 */
export function PlanningExperience({ prev, next }: PlanningExperienceProps) {
  return (
    <div className="bg-[var(--true-white)]">
      {/* ── Section 1 · Hero ── */}
      <section className={TYPE_HERO_SHELL}>
        <div className="mx-auto max-w-6xl">
          <p className="text-[0.7125rem] tracking-[0.22em] uppercase text-[var(--heritage-sage)]/82">
            Connected journey · 06
          </p>
          <h1 className="mt-8 font-heading text-[3.36rem] font-medium leading-[1.1] tracking-tight text-[var(--forest-sage)] md:text-[4.2rem] lg:text-[5.04rem]">
            Planning
          </h1>
          <p className="mt-4 max-w-4xl font-heading text-2xl italic text-[var(--forest-sage)]/80 md:text-3xl">
            Everyone planning together—not everyone planning separately.
          </p>
          <p className="mt-8 text-base leading-[1.7] text-[var(--forest-sage)]/70 md:text-lg">
            Couples, venues, and your team all contribute to the same planning experience—so every
            <br />
            update, decision, and detail stays connected from the first conversation to event day.
          </p>
          <WalkthroughCtas className="mt-14">
            <MarketingCta
              href="/product#connected-journey"
              label="Back to Journey"
              variant="ghost"
            />
          </WalkthroughCtas>
        </div>
      </section>

      {/* ── Section 2 · Emotional Photography ── */}
      <section className="px-6 pb-28 md:pb-36">
        <div className={`relative mx-auto aspect-[16/10] max-w-6xl md:aspect-[2/1] ${EDITORIAL_FRAME}`}>
          <Image
            src={FILM.planningPrep}
            alt="Hands writing a wedding plan with sage swatches and florals — collaboration as hospitality"
            fill
            className={EDITORIAL_IMAGE}
            sizes="100vw"
            priority
          />
        </div>
      </section>

      {/* ── Section 3 · Three Principles ── */}
      <section className="px-6 py-28 md:py-36">
        <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-3 md:gap-12">
          {PRINCIPLES.map((card) => (
            <div key={card.n} className="border-t border-[var(--taupe-medium)]/70 pt-8">
              <p className="font-heading text-sm text-[var(--heritage-sage)]/60">{card.n}</p>
              <h2 className="mt-4 font-heading text-2xl text-[var(--forest-sage)] md:text-[2.1rem]">
                {card.title}
              </h2>
              <p className="mt-4 text-sm leading-[1.7] text-[var(--forest-sage)]/70 md:text-base">
                {card.body}
              </p>
            </div>
          ))}
        </div>
      </section>

      {/* ── Section 4 · The Difference ── */}
      <section className="bg-[var(--warm-gray)] px-6 py-28 md:py-36">
        <div className="mx-auto grid max-w-6xl gap-14 md:grid-cols-2 md:items-start md:gap-20">
          <div>
            <h2 className="font-heading text-[2.1rem] font-medium leading-[1.21] text-[var(--forest-sage)] md:text-[3.36rem]">
              Planning becomes hospitality.
            </h2>
            <p className="mt-6 text-base leading-[1.7] text-[var(--forest-sage)]/75 md:text-lg max-w-[65ch]">
              Most planning software asks venues to become project managers.
            </p>
            <p className="mt-5 text-base leading-[1.7] text-[var(--forest-sage)]/75 md:text-lg max-w-[65ch]">
              Hello to Cheers lets them remain hosts.
            </p>
          </div>
          <div>
            <ol className="space-y-0">
              {FLOW.map((label, i) => (
                <li key={label} className="flex flex-col items-start">
                  <span className="font-heading text-xl text-[var(--forest-sage)] md:text-2xl">
                    {label}
                  </span>
                  {i < FLOW.length - 1 ? (
                    <span
                      className="my-2 pl-1 text-lg leading-none text-[var(--heritage-sage)]/45"
                      aria-hidden
                    >
                      ↓
                    </span>
                  ) : null}
                </li>
              ))}
            </ol>
          </div>
        </div>
      </section>

      {/* ── Section 5 · Product Showcase ── */}
      <section className="px-6 py-28 md:py-36">
        <div className="mx-auto grid max-w-6xl items-center gap-12 md:grid-cols-[1.35fr_0.65fr] md:gap-16">
          <div>
            <div
              className={`relative aspect-[1024/575] ${EDITORIAL_FRAME} bg-[var(--linen)]`}
            >
              <Image
                src="/marketing/planning-workspace-showcase-v2.jpg"
                alt="Planning progress for Elena & James — 68% complete with timeline, checklist, and tasks"
                fill
                className="object-contain object-center"
                sizes="(max-width:768px) 100vw, 720px"
              />
            </div>
          </div>
          <div>
            <h2 className="font-heading text-[2.1rem] text-[var(--forest-sage)] md:text-[2.52rem]">
              One place for every decision.
            </h2>
            <p className="mt-6 text-base leading-[1.7] text-[var(--forest-sage)]/70 md:text-lg max-w-[65ch]">
              Instead of asking, “Where did we write that?”
            </p>
            <p className="mt-5 text-base leading-[1.7] text-[var(--forest-sage)]/70 md:text-lg max-w-[65ch]">
              Your team already knows.
            </p>
            <p className="mt-5 text-base leading-[1.7] text-[var(--forest-sage)]/70 md:text-lg max-w-[65ch]">
              Because every decision stays with the event.
            </p>
          </div>
        </div>
      </section>

      {/* ── Section 6 · Hospitality Photography ── */}
      <section className={`px-6 ${EDITORIAL_BREAK_Y}`}>
        <div className={`relative mx-auto aspect-[16/10] max-w-6xl md:aspect-[2/1] ${EDITORIAL_FRAME}`}>
          <Image
            src={FILM.planningJoy}
            alt="Guests laughing at a candlelit reception — planning that strengthens relationships"
            fill
            className={EDITORIAL_IMAGE}
            sizes="100vw"
          />
        </div>
      </section>

      {/* ── Section 7 · The Hello to Cheers Difference ── */}
      <section className="px-6 py-28 md:py-36">
        <div className="mx-auto grid max-w-5xl gap-12 md:grid-cols-2 md:items-start md:gap-20">
          <h2 className="font-heading text-[2.1rem] text-[var(--forest-sage)] md:text-[3.36rem]">
            Less managing.
            <br />
            More celebrating.
          </h2>
          <div className="space-y-5 text-base leading-[1.7] text-[var(--forest-sage)]/75 md:text-lg max-w-[65ch]">
            <p>Planning doesn&apos;t belong to the venue.</p>
            <p>Or the couple.</p>
            <p>Or a coordinator.</p>
            <p>It belongs to everyone.</p>
            <p className="border-t border-[var(--taupe-medium)]/60 pt-8">
              That&apos;s why Hello to Cheers keeps every conversation, decision, timeline, task, and
              update together—so the experience feels collaborative instead of complicated.
            </p>
          </div>
        </div>
      </section>

      {/* ── Bottom Navigation ── */}
      <div className="pb-28 md:pb-36">
        <JourneyNav prev={prev} next={next} />
      </div>
    </div>
  );
}
