-- Fix stale-contact opportunity age in generate_venue_recommendations.
-- Brand-new leads with null last_contacted_at must not count as 7+ days without contact.
-- Age from coalesce(last_contacted_at, inquiry_date, created_at).
...
begin
  ...
  if not exists (
    ...
    where venue_id = v_venue_id
      ...
      and coalesce(last_contacted_at, inquiry_date::timestamptz, created_at)
          < (now() - interval '7 days');
