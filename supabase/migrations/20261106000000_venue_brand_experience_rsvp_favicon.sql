-- ============================================================================
-- Venue Brand Experience Phase 1 — get_rsvp_context exposes the venue's
-- name but not its logo, so the guest-facing /rsvp/[token] page has no way
-- to show a branded tab icon and instead inherits the app's own Wevenu
-- favicon. The RSVP page's on-page color system stays the couple's own
-- wedding-website accentColor (unchanged, by design — the Hosted Experience
-- is the couple's own aesthetic, not venue-branded) — this migration only
-- adds the logo for the browser-tab icon, which is chrome, not content.
-- Return type unchanged (jsonb), so this is a plain create-or-replace.
-- ============================================================================
...
begin
  ...
  return jsonb_build_object(
    ...
    'venue',       jsonb_build_object('name', v_venue.name, 'logoUrl', v_venue.logo_url),
