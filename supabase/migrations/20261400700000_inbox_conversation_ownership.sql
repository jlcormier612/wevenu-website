-- Inbox category = conversation ownership (inbox_owner_kind), NOT sales_stage / open-lead.
-- Dashboard Lead Flow remains a separate open-lifecycle metric.
-- Also repairs Sandbox Colby Yagnesak client wrongly attached to Ellie relationship.


alter table public.conversations
  add column if not exists inbox_owner_kind text,
  add column if not exists inbox_owner_lead_id uuid references public.leads(id) on delete set null,
  add column if not exists inbox_owner_client_id uuid references public.clients(id) on delete set null;

alter table public.conversations drop constraint if exists conversations_inbox_owner_kind_check;
alter table public.conversations
  add constraint conversations_inbox_owner_kind_check
  check (inbox_owner_kind is null or inbox_owner_kind in ('lead', 'client'));

comment on column public.conversations.inbox_owner_kind is
  'Inbox Leads vs Clients owner/source for venue_couple threads. Independent of sales_stage.';

-- Backfill venue_couple ownership from durable records (not open-stage filter).
-- Dual lead+client: lifecycle_booked_at ⇒ client channel; otherwise lead channel
-- (covers commercial-only clients that must stay under Leads).
with owners as (
  select
    ...
    (
      ...
      where l.relationship_id = c.relationship_id and l.venue_id = c.venue_id
      order by l.created_at desc
      limit 1
    ) as lead_id,
    (
      select cl.id from public.clients cl
      where cl.relationship_id = c.relationship_id and cl.venue_id = c.venue_id
      order by cl.created_at desc
      limit 1
    ) as client_id,
    (
      select cl.lifecycle_booked_at from public.clients cl
      where cl.relationship_id = c.relationship_id and cl.venue_id = c.venue_id
      order by cl.created_at desc
      limit 1
    ) as client_lifecycle_booked_at
  from public.conversations c
  where c.conversation_kind = 'venue_couple'
    and c.relationship_id is not null
)
update public.conversations c
set
  inbox_owner_kind = case
    when o.lead_id is not null and o.client_id is not null and o.client_lifecycle_booked_at is not null
      then 'client'
    when o.lead_id is not null then 'lead'
    when o.client_id is not null then 'client'
    else 'lead'
  end,
  inbox_owner_lead_id = case
    when o.lead_id is not null and o.client_id is not null and o.client_lifecycle_booked_at is not null
      then null
    when o.lead_id is not null then o.lead_id
    else null
  end,
  inbox_owner_client_id = case
    when o.lead_id is not null and o.client_id is not null and o.client_lifecycle_booked_at is not null
      then o.client_id
    when o.lead_id is null and o.client_id is not null then o.client_id
    else null
  end
from owners o
where c.id = o.conversation_id;

-- Prefer the client linked to the lead when stamping client ownership.
update public.conversations c
set inbox_owner_client_id = cl.id
from public.leads l
join public.clients cl on cl.lead_id = l.id and cl.venue_id = l.venue_id
where c.inbox_owner_kind = 'client'
  and c.relationship_id = l.relationship_id
  and c.venue_id = l.venue_id
  and c.conversation_kind = 'venue_couple'
  and (c.inbox_owner_client_id is distinct from cl.id);

-- Sandbox: move Colby Yagnesak client off Ellie relationship onto Colby lead relationship.
do $$
declare
...
begin
  ...
  from public.leads l
  where l.relationship_id = new.id and l.venue_id = new.venue_id
  order by l.created_at desc
  limit 1;

  select cl.id into v_client_id
  from public.clients cl
  where cl.relationship_id = new.id and cl.venue_id = new.venue_id
  order by cl.created_at desc
  limit 1;
  ...
  insert into public.conversations (
    venue_id, relationship_id, conversation_kind,
    inbox_owner_kind, inbox_owner_lead_id, inbox_owner_client_id
  )
  values (
    new.venue_id, new.id, 'venue_couple',
    v_owner,
    case when v_owner = 'lead' then v_lead_id else null end,
    case when v_owner = 'client' then v_client_id else null end
  )
...
-- When a lead is attached to a relationship, claim venue_couple as lead-owned
-- unless it is already a client-owned channel (lifecycle booking / booking file).
create or replace function public.stamp_conversation_owner_on_lead()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.conversations c
  set inbox_owner_kind = 'lead',
      inbox_owner_lead_id = new.id,
      inbox_owner_client_id = null
  where c.venue_id = new.venue_id
    and c.relationship_id = new.relationship_id
    and c.conversation_kind = 'venue_couple'
    and coalesce(c.inbox_owner_kind, 'lead') = 'lead';

  return new;
end;
$$;

drop trigger if exists leads_stamp_conversation_inbox_owner on public.leads;
create trigger leads_stamp_conversation_inbox_owner
  after insert or update of relationship_id
  on public.leads
  for each row
  execute function public.stamp_conversation_owner_on_lead();
