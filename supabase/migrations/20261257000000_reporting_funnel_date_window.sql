-- ============================================================================
-- Work Package R1 — Reporting Foundation.
--
-- canonical_conversion_funnel() (20261221000000) took no parameters —
-- all-time only. Every canonical revenue function already supports an
-- optional p_from/p_to date window; the funnel was the one canonical
-- metric a real Sales report couldn't date-filter at all, which would have
-- forced Reporting to either silently reinterpret it (the brief's own
-- explicit warning) or leave one report's numbers un-filterable while
-- every other report responds to the date range.
--
-- This is the SAME formula, parameterized — not a second funnel
-- implementation. Every stage now optionally filters to leads whose
-- created_at falls in [p_from, p_to] ("opportunities that entered the
-- pipeline in this period, and how far each one got") — one consistent
-- date semantic across all seven stages, per the brief's own "do not mix
-- date semantics without making the distinction explicit" rule (§11).
-- p_from/p_to both default to null (all-time), so every existing caller
-- (none yet — this RPC had no UI consumer before R1) is unaffected.
-- ============================================================================

-- D5E's own lesson relearned deliberately: CREATE OR REPLACE does not
-- replace a function whose parameter list differs — it silently creates a
-- second overload, and PostgREST then can't choose between them
-- (PGRST203). Drop the old 0-arg signature explicitly before recreating.
drop function if exists public.canonical_conversion_funnel();
...
begin
  ...
  from public.leads
  where venue_id = v_venue_id and status in ('proposal_sent', 'won')
    and (p_from is null or created_at::date >= p_from)
    and (p_to   is null or created_at::date <= p_to);
...
-- get_venue_analytics() calls canonical_conversion_funnel() with zero args
-- internally (20261221000000) — that exact signature no longer exists now
-- that the 0-arg overload was dropped above. Explicit nulls = the same
-- all-time behavior it already had; nothing else in this function changes.
