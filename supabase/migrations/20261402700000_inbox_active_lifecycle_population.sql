-- Inbox working populations follow the lead/client lifecycle, not a stale owner stamp.
-- Leads = open sales_stage on the relationship.
-- Clients = booked, not cancelled, and no open lead.
-- Lost, cancelled, and relationship-only certification threads stay out of both tabs.
-- Header unread / needs_response count only those working populations plus vendor threads.
  ...
  select exists (
    select 1
     ...
     where c.id = p_conversation_id
       ...
       and (
         c.conversation_kind in ('venue_vendor', 'couple_vendor', 'couple_vendor_inquiry')
         or exists (
