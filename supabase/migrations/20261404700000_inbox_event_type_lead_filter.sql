-- Inbox Event Type filter: match lead.event_type for Leads-tab / lead-owned threads
-- even when a client row exists on the relationship (client_id is coalesced).
-- Presentation/filter logic only — does not change population, ownership, or stored types.
    ...
    filtered as (
      ...
      where e.latest_message is not null
        ...
        and (
          ...
          -- Inbox Leads = an open sales lead on this relationship.
          -- Booked, lost, cancelled, and relationship-only threads are not active Leads.
          or (
            p_relationship in ('leads', 'lead')
            and exists (
              select 1 from public.leads ol
              where ol.venue_id = v_venue_id
                and ol.relationship_id = e.relationship_id
                and lower(coalesce(ol.sales_stage, '')) not in ('booked', 'lost', 'won', 'cancelled')
            )
          )
          -- Inbox Clients = a booked, non-cancelled client, and no open lead.
          or (
        ...
        and (
          ...
          or (
            -- Lead event-type / preferred-date filters must use leads.* even when a
            -- client row exists on the same relationship (coalesced client_id).
            -- Otherwise Event Type filters silently drop open-lead conversations
            -- that have no events yet (common for Inbox Leads).
            e.lead_id is not null
            and (
              e.client_id is null
              or e.inbox_owner_kind = 'lead'
              or p_relationship in ('leads', 'lead')
            )
            and (
              v_event_types is null
