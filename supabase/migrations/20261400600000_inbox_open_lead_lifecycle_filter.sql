-- Inbox relationship filter: open-lead lifecycle, not client_id presence.
-- Open lead + commercial-only client row stays in Leads.
-- Clients = client present AND no open lead on the relationship.
    ...
    filtered as (
      ...
      where e.latest_message is not null
        ...
        and (
          ...
          -- Inbox Leads = open commercial lifecycle (not booked/lost/won/cancelled),
          -- even when a commercial-only client row exists on the relationship.
          or (
            p_relationship in ('leads', 'lead')
            and exists (
              select 1 from public.leads ol
              where ol.relationship_id = e.relationship_id
                and ol.venue_id = v_venue_id
                and lower(coalesce(ol.sales_stage, '')) not in ('booked', 'lost', 'won', 'cancelled')
            )
          )
          -- Inbox Clients = has a client and no open lead on the relationship.
          or (
