-- Portal payments privacy: payment_line_items.notes are venue-internal
-- mark-paid / operational notes. They must not be returned to the couple
-- portal. Schedule-level notes (payment_schedules.notes) remain customer-
-- facing ("Notes from your venue") and stay in the payload.
...
comment on function public.get_portal_payments(text) is
  'Couple portal payment schedules. Exposes schedule.notes (customer-facing). Omits payment_line_items.notes (venue-internal mark-paid notes).';
