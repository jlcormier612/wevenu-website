...
-- ============================================================================
-- Additional fix found during this reconciliation (not in the original
-- branch): canonical_conversion_funnel_leads() was added later, in
-- 20261258000000_reporting_funnel_stage_detail.sql — after this branch's
-- migrations were originally written — and still reads/filters on the
-- deprecated leads.status column ('proposal_sent'/'won', excluding only
-- 'cancelled'). leads.status is no longer written by ingest_lead as of this
-- reconciliation, so left unfixed this function would silently return status
-- = null and an unfiltered lead list for every lead created from this point
-- forward. Remapped to sales_stage with the equivalent seven-stage values;
-- 'cancelled' and the prior 'lost' both collapse to sales_stage = 'lost', so
-- excluding 'lost' is the closest available equivalent to the prior
-- exclude-only-cancelled filter.
-- ============================================================================
